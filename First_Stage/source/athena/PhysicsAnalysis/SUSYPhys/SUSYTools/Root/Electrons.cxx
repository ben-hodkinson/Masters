...
// MATT
double SUSYObjDef_xAOD::GetTotalEleTriggerSF(const xAOD::ElectronContainer& electrons, const std::string& trigExpr) {
  ...
  if(trigExpr==singleLepStr) {
    ...
    ATH_MSG_WARNING( "Only single lepton trigger SFs are supported in GetTotalEleTriggerSF(). Use GetTriggerGlobalEfficiencySF() for dilepton or multilepton triggers!");
  }

  // Selecting the candidate electrons
  ConstDataVector<xAOD::ElectronContainer> sf_electrons(SG::VIEW_ELEMENTS);
  for (const auto& electron : electrons) {
    if( !acc_passOR(*electron) ) continue;
    if( !acc_signal(*electron) ) continue;
    sf_electrons.push_back(electron);
  }

  // Calculating the total SF based on Bayes and efficiencies
  double trig_sf    = 1.0;
  double dataFactor = 1.0;
  double mcFactor   = 1.0;
  for (const auto& electron : sf_electrons) {
    dataFactor *= ( 1 - GetEleTriggerEfficiency(*electron, theExpr) * GetEleTriggerEfficiencySF(*electron, theExpr) );
    mcFactor   *= ( 1 - GetEleTriggerEfficiency(*electron, theExpr) );
  }

...
// Initialise MET tools
  ...
  if (!m_metMaker.isUserConfigured()) {
    ...
    ATH_CHECK( m_metMaker.setProperty("JetSelection", "Tight") );
    if (m_jetInputType == xAOD::JetInput::EMPFlow) {
  ...
  // MATT: Loose MET maker tool
  m_metMaker_loose.setTypeAndName("met::METMaker/METMakerLS_SUSYTools");

  ATH_CHECK( m_metMaker_loose.setProperty("ORCaloTaggedMuons", m_metRemoveOverlappingCaloTaggedMuons) );
  ATH_CHECK( m_metMaker_loose.setProperty("DoSetMuonJetEMScale", m_metDoSetMuonJetEMScale) );
  ATH_CHECK( m_metMaker_loose.setProperty("DoRemoveMuonJets", m_metDoRemoveMuonJets) );
  ATH_CHECK( m_metMaker_loose.setProperty("UseGhostMuons", m_metUseGhostMuons) );
  ATH_CHECK( m_metMaker_loose.setProperty("DoMuonEloss", m_metDoMuonEloss) );
  ATH_CHECK( m_metMaker_loose.setProperty("GreedyPhotons", m_metGreedyPhotons) );
  ATH_CHECK( m_metMaker_loose.setProperty("VeryGreedyPhotons", m_metVeryGreedyPhotons) );

  ATH_CHECK( m_metMaker_loose.setProperty("JetSelection", "Loose") );
  if (m_jetInputType == xAOD::JetInput::EMPFlow) {
    ATH_CHECK( m_metMaker_loose.setProperty("DoPFlow", true) );
  }

  ATH_CHECK( m_metMaker_loose.retrieve() );

  // MATT: FJVT MET maker tool
  m_metMaker_fjvt.setTypeAndName("met::METMaker/METMakerJV_SUSYTools");

  ATH_CHECK( m_metMaker_fjvt.setProperty("ORCaloTaggedMuons", m_metRemoveOverlappingCaloTaggedMuons) );
  ATH_CHECK( m_metMaker_fjvt.setProperty("DoSetMuonJetEMScale", m_metDoSetMuonJetEMScale) );
  ATH_CHECK( m_metMaker_fjvt.setProperty("DoRemoveMuonJets", m_metDoRemoveMuonJets) );
  ATH_CHECK( m_metMaker_fjvt.setProperty("UseGhostMuons", m_metUseGhostMuons) );
  ATH_CHECK( m_metMaker_fjvt.setProperty("DoMuonEloss", m_metDoMuonEloss) );
  ATH_CHECK( m_metMaker_fjvt.setProperty("GreedyPhotons", m_metGreedyPhotons) );
  ATH_CHECK( m_metMaker_fjvt.setProperty("VeryGreedyPhotons", m_metVeryGreedyPhotons) );

  ATH_CHECK( m_metMaker_fjvt.setProperty("JetSelection", "Loose") );
  ATH_CHECK( m_metMaker_fjvt.setProperty("JetRejectionDec", "passFJvt") );
  if (m_jetInputType == xAOD::JetInput::EMPFlow) {
    ATH_CHECK( m_metMaker_fjvt.setProperty("DoPFlow", true) );
  }
